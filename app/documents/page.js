                    ...
                    {mydocuments.length == 0 ? (<div className='addDocoments text-center text-lg'>No documents available yet</div>) : (
                        mydocuments.map((element) => (
                            <div key={element._id} className='documentelement w-full py-4 rounded-md border-2 hover:bg-[#f7f7f7] border-gray-300 flex mb-4'>
                                ...
                                <div className='delete w-[15%] flex justify-center items-center text-sm'><button onClick={() => { setshowDeletePopup(element._id) }} className='px-2 py-1 flex justify-center items-center gap-1 bg-gray-100 border-2 border-gray-300 cursor-pointer rounded-md'><Image src={'/delete.svg'} alt='dlt' width={20} height={20} />Delete</button></div>
